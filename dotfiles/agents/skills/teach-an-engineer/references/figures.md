...
- Build each figure as HTML <div>s laid out with CSS grid/flex, so the math renderer (KaTeX auto-render in the templates) typesets equations natively inside them; overlay a small inline <svg> only for geometric primitives like axes, circles, and arrows — never put text or math inside an SVG.
...
- **Keep math in the HTML layer.** Let KaTeX auto-render typeset math as HTML/CSS spans so no math sits inside the SVG. For a truly self-contained file, embed the KaTeX CSS/JS and web fonts (e.g. WOFF2 as data URIs); this adds file size, so decide up front.
